...
> 🔒 **v2.1 安全變更**:Telegram Bot Token 已從 `index.html` 完全移除。
> 原因:GitHub Pages 是**公開網頁**,任何人打開原始碼都能看到 Token 並接管機器人。
> 現在 Token 只存在於 Apps Script(私有,只有您看得到)。
>
> **⚠ 請務必先換一組新 Token**(舊的已經公開過):
> 1. Telegram 找 **@BotFather** → `/revoke` → 選 `acadminbot` → 取得新 Token
> 2. 把新 Token 貼到 `AC_ADMIN_CENTER.gs` 最上方那行的引號裡:
>    `var BOT_TOKEN = getProp_('BOT_TOKEN') || '這裡貼新Token';`
...
4. 執行 `resetBot()` → 清空舊佇列並綁定 webhook(群組會收到確認訊息)
5. (選用)執行 `setupDailyDigest()` → 每日 08:00 自動發送全平台摘要

> ⚠️ 照舊規範:每次重新部署 GAS 後,務必再執行一次 `fixWebhook()`(或 `resetBot()`)。

### 3. Admin Center 前端

開啟 https://saintdou-weng.github.io/ac-admin/ 即可 —
✅ GAS URL **已預填**,不用設定。Settings 已**沒有** Token 欄位(那是刻意的,為了安全)。
到 **Settings** 按 **測試雲端**、**測試 Telegram** 確認即完成
(測試 Telegram 是請 GAS 後端代發,前端完全不接觸 Token)。
